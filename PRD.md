...
**感知**：Agent 在 System Prompt 中看到 available_skills 列表。

**决策**：当用户请求"查询北京天气"时，Agent 发现 get_weather 技能匹配。

**行动 (Tool Call)**：Agent **不调用** get_weather() 函数（因为它不存在），而是调用 **read_file(path="./backend/skills/get_weather/SKILL.md")**。

**学习与执行**：Agent 读取 Markdown 内容，理解操作步骤（例如："使用 fetch_url 访问某天气 API" 或 "使用 python_repl 运行以下代码"），然后**动态调用 Core Tools** (Terminal/Python) 来完成任务。
...
右侧：**"天行者"** (链接至 https://github.com/tianxingzhe-source)。
